...
```json
{
  "Text": "Фантастические твари: Преступления Грин-де-Вальда"
}
```
...
- `PortName` - COM-порт, к которому подключен принтер. Текстовая строка с названием порта, например, `COM1`, `COM2` и т.д. Для создания виртуального порта можно использовать утилиту **Custom-RU Test Tool**.

- `OperatorPassword` - пароль оператора принтера. Текстовая строка.


### Секция `Slip` — настройки для слип-чека

- `LineSeparators` - разделители строк в слип-чеке, каждый из которых должен быть заменен на перенос строки перед печатью переданного текста в слип-чеке (см. описание http-запроса печати слип-чека).
